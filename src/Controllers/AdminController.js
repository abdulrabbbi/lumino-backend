...
export const changeTestUserPassword = async (req, res) => {
  try {
    ...
    if (!admin || admin.role !== "admin") {
      return res
        .status(403)
        .json({ success: false, message: "Only admin can change passwords" });
    }
    ...
    res
      ...
      .json({
        success: true,
        message: "Password updated successfully for test user",
      });
  } catch (error) {
...
export const deleteParentsActivity = async (req, res) => {
  try {
    ...
    if (!admin || admin.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Forbidden: only admin can delete activity" });
    }
  ...
  try {
    ...
    const activitiesWithCount = await Promise.all(
      activities.map(async (activity) => {
        const count = await CompletedActivity.countDocuments({
...
export const getPlatformStats = async (req, res) => {
  try {
    ...
    const newSubscribers = await User.countDocuments({
      createdAt: { $gte: getDate30DaysAgo() },
    });
...
export const createActivity = async (req, res) => {
  try {
    ...
    if (!user || user.role !== "admin")
      return res.status(404).json({ error: "Only admin has access" });
...
export const createBulkActivities = async (req, res) => {
  try {
    ...
    if (!creatorName || !learningDomain) {
      return res
        .status(400)
        .json({ error: "Creator name and learning domain are required" });
    }

    const validActivities = bulkActivities.filter(
...
export const getUserDetailsWithActivities = async (req, res) => {
  try {
    ...
    const formattedActivities = completedActivities
      .map((entry) => ({
